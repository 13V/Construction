...
- **Use a real map image, but never a live map embed.** A Google Maps script won't load — the page runs under a CSP that blocks external requests, and an unbilled key renders "For development purposes only" watermarks. Instead pull a **Static Maps API** PNG of the real area, desaturated, and embed it as a base64 `data:` URI. Real street names sell far better than an abstraction. Draw all markers, geofence circles and breadcrumbs as an **SVG overlay on top** — the static API can't render avatar pins with status rings. See [MAPS.md](MAPS.md) for the exact request and the production build. If no map image is supplied, fall back to a stylized SVG map: soft gray parcels, thin white road lines, a few street labels.
